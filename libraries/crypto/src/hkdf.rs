...
mod test {
    ...
    fn test_hkdf_256_sha256_vectors() {
        ...
        for (i, okm) in test_okms.iter().enumerate() {
            ...
            let info = [i as u8];
            assert_eq!(
                &hkdf_256::<Sha256>(&ikm.as_bytes(), &info[..]),
                array_ref!(okm, 0, 32)
            );
        }
    }
    ...
    fn test_hkdf_compare_implementations() {
        ...
        for _ in 0..128 {
            assert_eq!(
                hkdf::<Sha256>(&salt, input.as_slice(), l, input.as_slice()),
                hkdf_256::<Sha256>(input.as_slice(), input.as_slice())
            );
            input.push(b'A');
        }
    }
