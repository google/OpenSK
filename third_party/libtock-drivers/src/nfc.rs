...
pub mod nfc_helper {
    ...
    fn transmit_packet(console: &mut Console, buffer: &mut [u8]) -> ReturnCode {
        console.flush();
        ...
        match NfcTag::transmit(&mut buffer[..len]) {
            Ok(_) => {
                writeln!(
                    console,
                    "************> TX SUCCESSFUL ({} bytes): {:02x?}",
                    len,
                    &buffer[..len]
                )
                .unwrap();
            ...
            Err(e) => {
                writeln!(console, "************> TX Err: {:?}", e).unwrap();
    ...
    fn receive_packet(
        ...
        match NfcTag::receive(&mut buf) {
            Ok(RecvOp {
                ...
                writeln!(console, "************> RX: {:02x?}", &buf[..amount]).unwrap();
                console.flush();
            ...
            Err(e) => {
                writeln!(console, "************> RX Err: {:?}", e).unwrap();
                return (ReturnCode::ECANCEL, 0);
            }
        }
    }

    pub fn nfc_receive<F>(mut console: &mut Console, mut cb: F) -> bool
    where
    ...
    pub fn nfc_transmit<F>(mut console: &mut Console, buffer: &mut [u8], mut cb: F) -> bool
    where
        F: FnMut(ReturnCode),
    {
        // Setup the timer with a dummy callback (we only care about reading the current time, but the
        ...
        writeln!(console, "************> NFC TX: {:02x?}", buffer).unwrap();
        console.flush();

        loop {
            let retcode = transmit_packet(&mut console, buffer);
            ...
            match retcode {
                ...
                | ReturnCode::ENOSUPPORT => {}
                ReturnCode::SUCCESS => {
                    writeln!(console, "************> NFC TX RETCODE: {:?}", retcode).unwrap();
                    console.flush();
                    cb(retcode);
                    return true;
                }
...
impl NfcTag {
    ...
    pub fn transmit(buf: &mut [u8]) -> TockResult<usize> {
        let amount = buf.len();
    ...
    pub fn transmit_bytes(buf: &mut [u8]) {
        nfc_helper::nfc_transmit(&mut Console::new(), buf, |_| {});
    }
}
...
impl Transport for NfcTag {
    ...
    fn recv(&self, buf: &mut [u8]) -> bool {
        ...
        writeln!(console, "************> RECEIVING RESPONSE VIA NFC").unwrap();
        nfc_helper::nfc_receive(&mut Console::new(), |rx_buf, amount| {
            buf[..amount].copy_from_slice(&rx_buf[..amount])
        })
    }
    ...
    fn send(&self, buf: &mut [u8]) -> bool {
        let mut console = Console::new();
        writeln!(console, "************> TRANSMITTING RESPONSE VIA NFC").unwrap();
        match NfcTag::transmit(buf) {
            Ok(bytes_sent) => {
    ...
    fn recv_with_timeout(
        ...
        if self.recv(buf) {
            return Some(SendOrRecvStatus::Received);
        } else {
            return None;
        }
    }
