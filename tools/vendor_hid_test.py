...
class HidDevice(object):
    ...
    def ping_init(self, packets=1, byte=0x88) -> int:
        ...
        ping_packet = (
            [0] + list(self.cid) + [0x81, size // 256, size % 256] + [byte] * 57
        )
    ...
    def ping_continue(self, num, byte=0x88) -> int:
        ...
        assert len(continue_packet) == _SEND_DATA_SIZE, (
            f"Expected packet to be {_SEND_DATA_SIZE} but was {{len(continue_packet)}}"
        )
...
class HidInterfaces(unittest.TestCase):
    ...
    def _test_interleaved_send_and_batch_receive(self, a: HidDevice, b: HidDevice):
        byte_a = get_byte()
        byte_b = get_byte()
        r = a.ping_init(packets=_PACKETS, byte=byte_a)
        self.assertEqual(r, _SEND_DATA_SIZE)
    ...
    def _test_batch_send_and_interleaved_receive(self, a: HidDevice, b: HidDevice):
        byte_a = get_byte()
        byte_b = get_byte()
        r = a.ping_init(packets=_PACKETS, byte=byte_a)
        self.assertEqual(r, _SEND_DATA_SIZE)
...
class CliInteraction(UserInteraction):
    ...
    def prompt_up(self) -> None:
        ...
        if self.connection and self.cid:
            cancel_packet = (
                self.cid.to_bytes(4, byteorder="big")
                + b"\x91"
                + b"".join([b"\x00"] * 59)
            )
            assert len(cancel_packet) == _PACKET_SIZE, (
                f"Expected packet to be {_PACKET_SIZE} but was {{len(cancel_packet)}}"
            )
...
class CancelTests(unittest.TestCase):
    ...
    def test_cancel_works(self):
        ...
        self.assertEqual(
            context.exception.cause.code, ctap.CtapError.ERR.KEEPALIVE_CANCEL
        )
    ...
    def test_cancel_ignores_wrong_cid(self):
        ...
        self.assertEqual(context.exception.code, ClientError.ERR.TIMEOUT)
        self.assertEqual(
            context.exception.cause.code, ctap.CtapError.ERR.USER_ACTION_TIMEOUT
        )
